...
-- Calculates the average cost per night for a gold customer.
...
JOIN has_rate rate 
    ...
    AND rate.season_id = s.id
JOIN discount d 
    ON d.discount_category = 'gold'
WHERE s.name = 'summer'
    AND rate.day_of_week IN ('wednesday', 'thursday')
    AND rt.id NOT IN (
        SELECT r2.room_type_id
        FROM rooms r2
        JOIN reservations_rooms rr 
            ON rr.room_number = r2.room_number 
...
VALUES (99001, 556677, 'passport', '45 Newbury St, Boston, MA', '617-443-9910', '617-443-0099', 'gold', 'Fanizza Tahir');

INSERT INTO reservations (idReservations, start, "end", room_type_name)
VALUES (99101, '2026-07-15', '2026-07-17', 'suite');
